...
if [ -f /usr/local/bin/ha_entrypoint.sh ]; then
    chown -R "$PUID:$PGID" /usr/local/bin/ha_entrypoint.sh
    chmod -R 755 /usr/local/bin/ha_entrypoint.sh
fi
...
# Define shebang
sed -i "s|/command/with-contenv bashio|$shebang|g" /usr/local/bin/ha_entrypoint.sh
