...
case $(bashio::config 'DB_CONNECTION') in
    ...
    sqlite_internal)
        ...
        if [ ! -f /config/addons_config/fireflyiii/database/database.sqlite ]; then
            ...
            #php /var/www/html/artisan migrate:refresh --seed --quiet
            php /var/www/html/artisan firefly-iii:upgrade-database --quiet
            php /var/www/html/artisan passport:install --quiet
        fi
